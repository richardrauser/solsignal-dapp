...
function getSolanaConnection() {
  const solanaConnection = new Connection(SOLANA_HTTP_ENDPOINT, { wsEndpoint: SOLANA_WSS_ENDPOINT })
...
export async function fetchWalletTransactions(walletAddress: string) {
  ...
  const sigList = await solanaConnection.getSignaturesForAddress(walletPublicKey, { limit: 5 })
...
export function createWalletSubscription(walletAddress: string, callback: AccountChangeCallback) {
  ...
  return (async () => {
    ...
    const subscriptionId = solanaConnection.onAccountChange(walletPublicKey, callback, config)
